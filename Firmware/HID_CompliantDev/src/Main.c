...
const uint8_t
    MyManuInfo[] = {0x30, 0x03, 'M', 0, 'o', 0, 'y', 0, 'u', 0, ' ', 0, 'a', 0,
                    ...
                    'o',  0,    'g', 0, 'y', 0};  // MoYu at work Technology
// 产品信息
const uint8_t MyProdInfo[] = {0x1C, 0x03, 'K', 0, 'V', 0, 'M', 0, ' ', 0,
                              'C',  0,    'a', 0, 'r', 0, 'd', 0, ' ', 0,
                              'M',  0,    'i', 0, 'n', 0, 'i', 0};
...
uint8_t EP0_Databuf[64 + 64 + 64];  // ep0(64)+ep4_out(64)+ep4_in(64)
__attribute__((aligned(4)))
uint8_t EP1_Databuf[64 + 64];  // ep1_out(64)+ep1_in(64)
__attribute__((aligned(4)))
uint8_t EP2_Databuf[64 + 64];  // ep2_out(64)+ep2_in(64)
__attribute__((aligned(4)))
uint8_t EP3_Databuf[64 + 64];  // ep3_out(64)+ep3_in(64)
...
const uint8_t U2KeyRepDesc[] = {
    ...
    0x95, 0x01, 0x75, 0x08, 0x81, 0x01, 0x95, 0x03, 0x75, 0x01, 0x05,
    0x08, 0x19, 0x01, 0x29, 0x03, 0x91, 0x02, 0x95, 0x05, 0x75, 0x01,
...
/******** 用户自定义分配端点RAM ****************************************/
__attribute__((aligned(4)))
uint8_t U2EP0_Databuf[64 + 64 + 64];  // ep0(64)+ep4_out(64)+ep4_in(64)
__attribute__((aligned(4)))
uint8_t U2EP1_Databuf[64 + 64];  // ep1_out(64)+ep1_in(64)
__attribute__((aligned(4)))
uint8_t U2EP2_Databuf[64 + 64];  // ep2_out(64)+ep2_in(64)
__attribute__((aligned(4)))
uint8_t U2EP3_Databuf[64 + 64];  // ep3_out(64)+ep3_in(64)
...
{
  uint8_t len,
      chtype;  // len用于拷贝函数，chtype用于存放数据传输方向、命令的类型、接收的对象等信息
  uint8_t intflag,
      errflag =
          0;  // intflag用于存放标志寄存器值，errflag用于标记是否支持主机的指令

  intflag = R8_USB_INT_FG;  // 取得中断标识寄存器的值

  if (intflag &
      RB_UIF_TRANSFER)  // 判断_INT_FG中的USB传输完成中断标志位。若有传输完成中断了，进if语句
  {
    if ((R8_USB_INT_ST & MASK_UIS_TOKEN) !=
        MASK_UIS_TOKEN)  // 非空闲
                         // //判断中断状态寄存器中的5:4位，查看令牌的PID标识。若这两位不是11（表示空闲），进if语句
    {
      switch (
          R8_USB_INT_ST &
          (MASK_UIS_TOKEN |
           MASK_UIS_ENDP))  // 取得令牌的PID标识和设备模式下的3:0位的端点号。主机模式下3:0位是应答PID标识位
      // 分析操作令牌和端点号
      {  // 端点0用于控制传输。以下的端点0的IN和OUT令牌相应程序，对应控制传输的数据阶段和状态阶段。
        ...
        {  // 端点0为双向端点，用作控制传输。 “|0”运算省略了
          switch (
              SetupReqCode)  // 这个值会在收到SETUP包时赋值。在后面会有SETUP包处理程序，对应控制传输的设置阶段。
          {
            case USB_GET_DESCRIPTOR:  // USB标准命令，主机从USB设备获取描述
              len =
                  SetupReqLen >= DevEP0SIZE
                      ? DevEP0SIZE
                      : SetupReqLen;  // 本次包传输长度。最长为64字节，超过64字节的分多次处理，前几次要满包。
              memcpy(
                  pEP0_DataBuf, pDescr,
                  len);  // memcpy:内存拷贝函数，从(二号位)地址拷贝(三号位)字符串长度到(一号位)地址中
              ...
              R8_UEP0_CTRL ^=
                  RB_UEP_T_TOG;  // 同步切换。IN方向（对于单片机就是T方向）的PID中的DATA0和DATA1切换
            ...
            default:
              R8_UEP0_T_LEN =
                  0;  // 状态阶段完成中断或者是强制上传0长度数据包结束控制传输（数据字段长度为0的数据包，包里SYNC、PID、EOP字段都有）
              R8_UEP0_CTRL = UEP_R_RES_ACK | UEP_T_RES_NAK;
        ...
        case UIS_TOKEN_OUT | 1:  // 令牌包的PID为OUT，端点号为1
        {
          ...
          {                   // 不同步的数据包将丢弃
            R8_UEP1_CTRL ^=
                RB_UEP_R_TOG;  // OUT事务的DATA同步切换。设定一个期望值。
            len = R8_USB_RX_LEN;  // 读取接收数据的字节数
            DevEP1_OUT_Deal(
                len);  // 发送长度为len的字节，自动回ACK握手包。自定义的程序。
          }
        } break;

        case UIS_TOKEN_IN | 1:  // 令牌包的PID为IN，端点号为1
          R8_UEP1_CTRL ^=
              RB_UEP_T_TOG;  // IN事务的DATA切换一下。设定将要发送的包的PID。
          R8_UEP1_CTRL =
              (R8_UEP1_CTRL & ~MASK_UEP_T_RES) |
              UEP_T_RES_NAK;  // 当DMA中没有由单片机更新数据时，将T响应IN事务置为NAK。更新了就发出数据。
          Ready = 1;
#if DEBUG_PRT
    ...
    if (R8_USB_INT_ST & RB_UIS_SETUP_ACT)  // Setup包处理
    {
      ...
      SetupReqLen =
          pSetupReqPak
              ->wLength;  // 数据阶段的字节数
                          // //pSetupReqPak：将端点0的RAM地址强制转换成一个存放结构体的地址，结构体成员依次紧凑排列
      SetupReqCode = pSetupReqPak->bRequest;  // 命令的序号
      chtype =
          pSetupReqPak
              ->bRequestType;  // 包含数据传输方向、命令的类型、接收的对象等信息

      len = 0;
      errflag = 0;
      if ((pSetupReqPak->bRequestType & USB_REQ_TYP_MASK) !=
          USB_REQ_TYP_STANDARD)  // 判断命令的类型，如果不是标准请求，进if语句
      {
        /* 非标准请求 */
        /* 其它请求,如类请求，产商请求等 */
      ...
      } else  // 判断为标准请求
      {
        switch (SetupReqCode)  // 判断命令的序号
        {
          case USB_GET_DESCRIPTOR:  // 主机想获得标准描述符
          {
            switch ((
                (pSetupReqPak->wValue) >>
                8))  // 右移8位，看原来的高8位是否为0，为1表示方向为IN方向，则进s-case语句
            {
              case USB_DESCR_TYP_DEVICE:  // 不同的值代表不同的命令。主机想获得设备描述符
              {
                pDescr =
                    MyDevDescr;  // 将设备描述符字符串放在pDescr地址中，“获得标准描述符”这个case末尾会用拷贝函数发送
                len = MyDevDescr
                    [0];  // 协议规定设备描述符的首字节存放字节数长度。拷贝函数会用到len参数
              } break;

              case USB_DESCR_TYP_CONFIG:  // 主机想获得配置描述符
              {
                pDescr =
                    MyCfgDescr;  // 将配置描述符字符串放在pDescr地址中，之后会发送
                len = MyCfgDescr
                    [2];  // 协议规定配置描述符的第三个字节存放配置信息的总长
              } break;
          ...
          case USB_SET_ADDRESS:  // 主机想设置设备地址
            SetupReqLen = (pSetupReqPak->wValue) &
                          0xff;  // 将主机分发的位设备地址暂存在SetupReqLen中
          ...
          case USB_GET_CONFIGURATION:     // 主机想获得设备当前配置
            ...
            if (SetupReqLen > 1)
              SetupReqLen =
                  1;  // 将数据阶段的字节数置1。因为DevConfig只有一个字节
            break;
          ...
          case USB_CLEAR_FEATURE:  // 关闭USB设备的特征/功能。可以是设备或是端点层面上的。
          {
                ...
                USB_REQ_RECIP_ENDP)  // 判断是不是端点特征（清除端点停止工作的状态）
            {
              ...
              {  // 16位的最高位判断数据传输方向，0为OUT，1为IN。低位为端点号。
                ...
                USB_REQ_RECIP_DEVICE)  // 判断是不是设备特征（用于设备唤醒）
            {
          ...
          case USB_SET_FEATURE:  // 开启USB设备的特征/功能。可以是设备或是端点层面上的。
            if ((pSetupReqPak->bRequestType & USB_REQ_RECIP_MASK) ==
                USB_REQ_RECIP_ENDP)  // 判断是不是端点特征（使端点停止工作）
            {
              /* 端点 */
              ...
              {  // 16位的最高位判断数据传输方向，0为OUT，1为IN。低位为端点号。
                ...
                (pSetupReqPak->bRequestType & USB_REQ_RECIP_MASK) ==
                USB_REQ_RECIP_DEVICE)  // 判断是不是设备特征（使设备休眠）
            {
          ...
          case USB_GET_STATUS:  // 主机想获得设备、接口或是端点的状态
            if ((pSetupReqPak->bRequestType & USB_REQ_RECIP_MASK) ==
                USB_REQ_RECIP_ENDP)  // 判断是否为端点状态
            {
              /* 端点 */
              pEP0_DataBuf[0] = 0x00;
              switch (
                  pSetupReqPak
                      ->wIndex) {  // 16位的最高位判断数据传输方向，0为OUT，1为IN。低位为端点号。
                case 0x81:  // 判断_TOG和_T_RES三位，若处于STALL状态，进if语句
                  if ((R8_UEP1_CTRL & (RB_UEP_T_TOG | MASK_UEP_T_RES)) ==
                      UEP_T_RES_STALL) {
                    pEP0_DataBuf[0] =
                        0x01;  // 返回D0为1，表示端点被停止工作了。该位由SET_FEATURE和CLEAR_FEATURE命令配置。
                  }
                  break;
                ...
                case 0x01:  // 判断_TOG和_R_RES三位，若处于STALL状态，进if语句
                  ...
                  break;
                       ...
                       USB_REQ_RECIP_DEVICE)  // 判断是否为设备状态
            {
              pEP0_DataBuf[0] = 0x00;
              if (USB_SleepStatus)  // 如果设备处于睡眠状态
              {
                pEP0_DataBuf[0] =
                    0x02;  // 最低位D0为0，表示设备由总线供电，为1表示设备自供电。
                           // D1位为1表示支持远程唤醒，为0表示不支持。
              } else {
                pEP0_DataBuf[0] = 0x00;
              }
            }
  ...
  {
    if (R8_USB_MIS_ST &
        RB_UMS_SUSPEND)  // 取得杂项状态寄存器中的挂起状态位，为1表示USB总线处于挂起态，为0表示总线处于非挂起态
    {
      Ready = 0;
#if DEBUG_PRT
...
void USB2_DevTransProcess(void) {
  ...
  if (intflag & RB_UIF_TRANSFER) {
    if ((R8_USB2_INT_ST & MASK_UIS_TOKEN) != MASK_UIS_TOKEN)  // 非空闲
    {
      ...
      // 分析操作令牌和端点号
      {
        case UIS_TOKEN_IN: {
          switch (U2SetupReqCode) {
            case USB_GET_DESCRIPTOR:
              len = U2SetupReqLen >= U2DevEP0SIZE
                        ? U2DevEP0SIZE
                        : U2SetupReqLen;             // 本次传输长度
        ...
        case UIS_TOKEN_OUT | 1: {
          if (R8_USB2_INT_ST & RB_UIS_TOG_OK) {  // 不同步的数据包将丢弃
    ...
    if (R8_USB2_INT_ST & RB_UIS_SETUP_ACT)  // Setup包处理
    {
      ...
      len = 0;
      errflag = 0;
      if ((pU2SetupReqPak->bRequestType & USB_REQ_TYP_MASK) !=
      ...
      } else /* 标准请求 */
      {
        switch (U2SetupReqCode) {
          case USB_GET_DESCRIPTOR: {
            ...
            if (U2SetupReqLen > len) U2SetupReqLen = len;  // 实际需上传总长度
            len =
                (U2SetupReqLen >= U2DevEP0SIZE) ? U2DevEP0SIZE : U2SetupReqLen;
          ...
          case USB_GET_CONFIGURATION:
            ...
            if (U2SetupReqLen > 1) U2SetupReqLen = 1;
            break;
          ...
          case USB_CLEAR_FEATURE: {
            if ((pU2SetupReqPak->bRequestType & USB_REQ_RECIP_MASK) ==
                USB_REQ_RECIP_ENDP)  // 端点
            {
              switch ((pU2SetupReqPak->wIndex) & 0xff) {
                case 0x83:
                  R8_U2EP3_CTRL =
                      (R8_U2EP3_CTRL & ~(RB_UEP_T_TOG | MASK_UEP_T_RES)) |
                      UEP_T_RES_NAK;
                  break;
                case 0x03:
                  R8_U2EP3_CTRL =
                      (R8_U2EP3_CTRL & ~(RB_UEP_R_TOG | MASK_UEP_R_RES)) |
                      UEP_R_RES_ACK;
                  break;
                case 0x82:
                  R8_U2EP2_CTRL =
                      (R8_U2EP2_CTRL & ~(RB_UEP_T_TOG | MASK_UEP_T_RES)) |
                      UEP_T_RES_NAK;
                  break;
                case 0x02:
                  R8_U2EP2_CTRL =
                      (R8_U2EP2_CTRL & ~(RB_UEP_R_TOG | MASK_UEP_R_RES)) |
                      UEP_R_RES_ACK;
                  break;
                case 0x81:
                  R8_U2EP1_CTRL =
                      (R8_U2EP1_CTRL & ~(RB_UEP_T_TOG | MASK_UEP_T_RES)) |
                      UEP_T_RES_NAK;
                  break;
                case 0x01:
                  R8_U2EP1_CTRL =
                      (R8_U2EP1_CTRL & ~(RB_UEP_R_TOG | MASK_UEP_R_RES)) |
                      UEP_R_RES_ACK;
                  break;
                default:
                  errflag = 0xFF;  // 不支持的端点
            ...
            } else if ((pU2SetupReqPak->bRequestType & USB_REQ_RECIP_MASK) ==
          ...
          case USB_GET_STATUS:
            if ((pU2SetupReqPak->bRequestType & USB_REQ_RECIP_MASK) ==
              ...
              switch (pU2SetupReqPak->wIndex) {
                case 0x83:
                  if ((R8_U2EP3_CTRL & (RB_UEP_T_TOG | MASK_UEP_T_RES)) ==
                      UEP_T_RES_STALL) {
                ...
                case 0x03:
                  if ((R8_U2EP3_CTRL & (RB_UEP_R_TOG | MASK_UEP_R_RES)) ==
                      UEP_R_RES_STALL) {
                    pU2EP0_DataBuf[0] = 0x01;
                  }
                  break;
                ...
                case 0x82:
                  if ((R8_U2EP2_CTRL & (RB_UEP_T_TOG | MASK_UEP_T_RES)) ==
                      UEP_T_RES_STALL) {
                    pU2EP0_DataBuf[0] = 0x01;
                  }
                  break;
                ...
                case 0x02:
                  if ((R8_U2EP2_CTRL & (RB_UEP_R_TOG | MASK_UEP_R_RES)) ==
                      UEP_R_RES_STALL) {
                    pU2EP0_DataBuf[0] = 0x01;
                  }
                  break;
                ...
                case 0x81:
                  if ((R8_U2EP1_CTRL & (RB_UEP_T_TOG | MASK_UEP_T_RES)) ==
                      UEP_T_RES_STALL) {
                    pU2EP0_DataBuf[0] = 0x01;
                  }
                  break;
                ...
                case 0x01:
                  if ((R8_U2EP1_CTRL & (RB_UEP_R_TOG | MASK_UEP_R_RES)) ==
                      UEP_R_RES_STALL) {
                    pU2EP0_DataBuf[0] = 0x01;
                  }
                  break;
      ...
      {
        //                  U2SetupReqCode = 0xFF;
        R8_U2EP0_CTRL = RB_UEP_R_TOG | RB_UEP_T_TOG | UEP_R_RES_STALL |
                        UEP_T_RES_STALL;  // STALL
      } else {
        if (chtype & 0x80)  // 上传
        {
          ...
          len = 0;  // 下传
        R8_U2EP0_T_LEN = len;
        R8_U2EP0_CTRL = RB_UEP_R_TOG | RB_UEP_T_TOG | UEP_R_RES_ACK |
                        UEP_T_RES_ACK;  // 默认数据包是DATA1
      }
  ...
  switch (pEP1_OUT_DataBuf[0]) {
    ...
    case 3:
      ...
      HID_Buf[2] = HIDKeyLightsCode;
	      HID_Buf[3] = 0;
	      if (g_mcu_ready)         HID_Buf[3] |= 0x01;
	      if (DevConfig)           HID_Buf[3] |= 0x02;
	      if (U2DevConfig)         HID_Buf[3] |= 0x04;
	      if (g_usb2_kbd_active)   HID_Buf[3] |= 0x08;
	      if (g_usb2_mouse_active) HID_Buf[3] |= 0x10;
	      HID_Buf[4] = g_led_auto_mode;
 ...
 * @return  none
 */
void U2DevEP1_OUT_Deal(uint8_t l) { /* 用户可自定义 */
  uint8_t i;
 ...
 * @return  none
 */
void U2DevEP2_OUT_Deal(uint8_t l) { /* 用户可自定义 */
  uint8_t i;
 ...
 * @return  none
 */
void U2DevEP3_OUT_Deal(uint8_t l) { /* 用户可自定义 */
  uint8_t i;
 ...
 * @return  none
 */
void U2DevEP4_OUT_Deal(uint8_t l) { /* 用户可自定义 */
  uint8_t i;
